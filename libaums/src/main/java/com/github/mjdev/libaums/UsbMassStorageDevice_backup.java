 ...
 *
 */
public class UsbMassStorageDevice_backup {

	private static final String TAG = UsbMassStorageDevice_backup.class.getSimpleName();
	...
	private UsbMassStorageDevice_backup(UsbManager usbManager, UsbDevice usbDevice,
	...
	public static UsbMassStorageDevice_backup[] getMassStorageDevices(Context context) {
		UsbManager usbManager = (UsbManager) context.getSystemService(Context.USB_SERVICE);
		ArrayList<UsbMassStorageDevice_backup> result = new ArrayList<UsbMassStorageDevice_backup>();

		for (UsbDevice device : usbManager.getDeviceList().values()) {
			CustomLog.i(TAG, "found usb device: " + device);
			...
			for (int i = 0; i < interfaceCount; i++) {
				...
				result.add(new UsbMassStorageDevice_backup(usbManager, device, usbInterface, inEndpoint, outEndpoint));
			}
		}

		return result.toArray(new UsbMassStorageDevice_backup[0]);
	}
	...
	public void close() {
		...
		if (!release) {
			CustomLog.e(TAG, "could not release interface!");
		}
